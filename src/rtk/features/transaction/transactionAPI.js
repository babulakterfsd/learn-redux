...
export const editTransaction = async (id, data) => {
    ...
    if (itemResponse.data.type === data.type) {
        const actionBalance = Number(response.data.amount) - Number(itemResponse.data.amount);
        if (data.type === 'income') {
        ...
        const actionBalance = Number(itemResponse.data.amount) + Number(data.amount);
...
export const deleteTransaction = async (id) => {
    ...
    const newBalance =
        itemResponse.data.type === 'income'
            ? Number(balanceResponse.data.totalBalance) - Number(itemResponse.data.amount)
            : Number(balanceResponse.data.totalBalance) + Number(itemResponse.data.amount);
    const updatedBalance = await axios.put('/balance', {
        totalBalance: newBalance,
    });
